...
import { fetchDataUrl } from '@/utils/Fetch'
...
function cloneChildNodes<T extends Node>(
  ...
  const firstChild =
    (isElement(node) ? node.shadowRoot?.firstChild : undefined) ??
    node.firstChild
...
export function cloneNode<T extends Node>(
  ...
  if (
    ownerWindow &&
    isElement(node) &&
    (isHTMLElement(node) || isSVGElement(node))
  ) {
    ...
    /** 处理图片任务 */
    if (isElement(node) && isImageElement(node)) {
